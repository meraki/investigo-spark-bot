...
def show():
    try:
        ...
        return render_template("meraki_server/list.html", object=servers)
...
@mod_meraki_server.route('/edit/<server_id>', methods=['GET', 'POST'])
def edit(server_id):
    output = None
    try:
        server = db_session.query(CMXServer).filter(CMXServer.id == server_id).first()
...
@mod_meraki_server.route('/verticalization/add/<server_id>', methods=['POST'])
...
@mod_meraki_server.route('/verticalization/remove/<server_id>', methods=['POST'])
